  ...
  extends FunSpec with Matchers with MockitoSugar with OneInstancePerTest
{
  ...
  describe("MagicManager") {
    ...
    describe("#applyDynamic") {
      describe("backwards comparability for magic invocation") {
        it("should bubble an exception if line magic execution throws an exception") {
          intercept[LineMagicException] {
            val magic = new ExceptionLineMagic
            doReturn(None).when(mockPluginManager).fireEventFirstResult(
              anyString(), any(classOf[Dependency[_ <: AnyRef]])
            )
            doReturn(Seq(magic).toIterable).when(mockPluginManager).plugins

            magicManager.applyDynamic(magic.getClass.getSimpleName)()
          }
        }

        it("should bubble an exception if cell magic execution throws an exception") {
          intercept[CellMagicException] {
            val magic = new ExceptionCellMagic
            doReturn(None).when(mockPluginManager).fireEventFirstResult(
              anyString(), any(classOf[Dependency[_ <: AnyRef]])
            )
            doReturn(Seq(magic)).when(mockPluginManager).plugins

            magicManager.applyDynamic(magic.getClass.getSimpleName)()
          }
        }

        it("should take the first argument and convert it to a string to pass to the line magic") {
          val expected = "some arg"

          val magic = new SomeLineMagic {
            var lastInput: String = _
            override def execute(code: String): LineMagicOutput = lastInput = code
          }
          ...
          doReturn(Seq(magic)).when(mockPluginManager).plugins
          magicManager.applyDynamic(magic.getClass.getSimpleName)(
            expected :: "other arg" :: Nil: _*
          )

          val actual = magic.lastInput
        ...
        it("should take the first argument and convert it to a string to pass to the cell magic") {
          val expected = "some arg"

          val magic = new SomeCellMagic {
            var lastInput: String = _
        ...
        it("should pass an empty string to the cell magic if no arguments are provided") {
          val expected = ""
        ...
        it("should return a Right[LineMagicOutput] if line magic execution is successful") {
          val magic = new SomeLineMagic
          doReturn(Seq(magic)).when(mockPluginManager).plugins
          ...
          val result = magicManager.applyDynamic(magic.getClass.getSimpleName)()
          result.right.get shouldBe a [scala.runtime.BoxedUnit]
        }

        it("should return a Left[CellMagicOutput] if cell magic execution is successful") {
          val expected = TestCellMagicOutput
        ...
        it("should return a Left[CellMagicOutput] if is a magic but not a line or cell") {
          ...
          val result = magicManager.applyDynamic(magic.getClass.getSimpleName)()
          result.left.get shouldBe a [CellMagicOutput]
        }
      }
