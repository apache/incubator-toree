...
if [[ "$RELEASE_PREPARE" == "true" ]]; then
    ...
    if [ -z "$DRY_RUN" ]; then
        ...
        # Create SVN staging folder locally for review
        mkdir -p dist/svn-release/$RELEASE_STAGING_FOLDER/apache-toree
        mkdir -p dist/svn-release/$RELEASE_STAGING_FOLDER/toree-pip
        mkdir -p dist/svn-release/$RELEASE_STAGING_FOLDER/apache-toree-pip

        cp dist/apache-toree-bin/* dist/svn-release/$RELEASE_STAGING_FOLDER/apache-toree/
        cp dist/apache-toree-src/* dist/svn-release/$RELEASE_STAGING_FOLDER/apache-toree/
        cp dist/toree-pip/*.tar.gz dist/toree-pip/*.asc dist/toree-pip/*.sha512 dist/svn-release/$RELEASE_STAGING_FOLDER/toree-pip/
        cp -r dist/toree-pip/toree.egg-info dist/svn-release/$RELEASE_STAGING_FOLDER/toree-pip/
        cp dist/apache-toree-pip/*.tar.gz dist/apache-toree-pip/*.asc dist/apache-toree-pip/*.sha512 dist/svn-release/$RELEASE_STAGING_FOLDER/apache-toree-pip/
        cp -r dist/apache-toree-pip/*toree.egg-info dist/svn-release/$RELEASE_STAGING_FOLDER/apache-toree-pip/
    fi
    ...
    echo "SVN staging folder is assembled at: $BASE_DIR/target/toree/dist/svn-release/$RELEASE_STAGING_FOLDER"
    echo ""
...
if [[ "$RELEASE_PUBLISH" == "true" ]]; then
    ...
    cp -r toree/dist/svn-release/$RELEASE_STAGING_FOLDER svn-toree/
